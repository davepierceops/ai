...
### ~~`bin/dispatch`~~ — retired 2026-08-09, not built
Would have enforced dispatch discipline chat-side: refuse to emit the dispatch block until the directive file was committed, and stamp the git-read SHA into it (Q2).

**Retired** by `docs/cycles/friction-refactor-2026-08-09-directive.md` (D1.1, D1.3). The directive now travels as a paste block and the executor lands it, so there is no chat-side commit to gate and no pre-dispatch SHA to stamp — the tool's entire premise was the transport that was removed. The executor-side equivalent is three git commands in a session that already has git, which does not warrant a tool. The two build triggers added 2026-08-06 were written against the `~/Downloads` delivery block, which is retired with it.
